...
async def generate_text(model: str, prompt: str, max_tokens: int = 8000, temperature: float = 0) -> str:
    ...
    elif model.startswith("meta-llama/") or model.startswith("deepseek-ai") or model.startswith(
            "Qwen/") or model.startswith("Meta-Llama"):
    ...
    # Add DeepSeek model support after the NVIDIA section
        ...
        if model == "deepseek-reasoner":
            ...
            # Combine reasoning and content for the reasoner model
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(api_url, headers=headers, json=payload) as response:
                    if response.status == 200:
                        ...
                        raise Exception(
                            f"Error with Hugging Face Inference API for model {model}: {await response.text()}")
