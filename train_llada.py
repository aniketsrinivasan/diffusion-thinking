...
def main():
    model_config = LLaDaModelConfig(
        ...
        freeze_transformer_blocks=list(range(0, 12)),
    )
    ...
    train_dataset_config = TUFAIntegralsDatasetConfig(
        ...
        dataset_paths=["data/integration/tufa_llama_variants_cleaned/cleaned_data_short_easier.json"],
    )
    ...
    config = LLaDaTrainerConfig(
        run_name="model_0_llada",
        num_epochs=20,
        batch_size=1,
        grpo_group_size=4,
        grpo_policy_update_steps=4,
        gradient_accumulation_steps=4,
        trajectory_sparsity=0.7,
        learning_rate=1e-6,
        validate_epochs=1,
        overwrite_run=True,
        project_dir="testing_runs",
